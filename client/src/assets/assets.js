...
/* File này chứa toàn bộ dữ liệu giả (dummy data) 
để bạn test giao diện website đặt vé phim, 
bao gồm hình ảnh, trailer, danh sách phim, diễn viên, lịch chiếu, 
dữ liệu dashboard admin và dữ liệu đặt vé của user. 
*/
export const assets = {
  logo,
  marvelLogo,
...
*/
// ... (code assets, dummyTrailers, dummyCastsData, dummyShowsData của bạn) ...

/* XÓA BỎ hoặc COMMENT LẠI 'dummyDateTimeData' CŨ NÀY:
...
// Hàm này tạo 6 ngày tiếp theo VÀ gán giờ chiếu giả cho mỗi ngày
export const generateDynamicDateTimeData = () => {
  const dates = {}; // 1. Vẫn tạo Object (như bạn muốn)
  const today = new Date();

  // Các giờ chiếu giả để dùng lại
  const dummyTimes = [
    { timeSuffix: "T09:00:00.000Z", showIdPrefix: "morning" },
    { timeSuffix: "T12:00:00.000Z", showIdPrefix: "noon" },
    { timeSuffix: "T15:00:00.000Z", showIdPrefix: "afternoon" },
    { timeSuffix: "T18:00:00.000Z", showIdPrefix: "evening" },
    { timeSuffix: "T21:00:00.000Z", showIdPrefix: "night" },
  ];
  ...
  for (let i = 0; i < 6; i++) {
    ...
    // 2. Gán mảng giờ chiếu THẬT SỰ (thay vì mảng rỗng [])
    dates[dateString] = dummyTimes.map((slot) => ({
      // Ghép ngày với giờ để tạo chuỗi ISO đầy đủ
      time: dateString + slot.timeSuffix,
      // Tạo showId duy nhất cho mỗi suất chiếu
      showId: `${dateString}-${slot.showIdPrefix}`,
    }));
  }

  return dates; // Trả về đối tượng: { "2025-11-18": [...], "2025-11-19": [...] }
};

// ... (code dummyDashboardData, dummyBookingData của bạn) ...
...
export const generateDynamicBookingData = () => {
  // 1. Lấy tất cả các suất chiếu động
  const allShows = generateDynamicDateTimeData();
  const allShowDates = Object.keys(allShows);

  const pricePerSeat = 8;

  if (allShowDates.length < 3) {
    return [];
  }

  // 2. Tạo 3 booking giả VỚI GIÁ $8

  // --- Booking 1: Hôm nay, suất 09:00, Phim "In the Lost Lands" (2 vé)
  const today = allShowDates[0];
  const show1 = allShows[today][0];
  const booking1 = {
    _id: "booking_dynamic_001",
    user: { name: "Tiến Dũng" },
    show: {
      _id: show1.showId,
      movie: dummyShowsData[0],
      showDateTime: show1.time,
      showPrice: pricePerSeat,
    },
    amount: pricePerSeat * 2,
    bookedSeats: ["C1", "C2"],
    isPaid: true,
  };

  // --- Booking 2: Ngày mai, suất 15:00, Phim "Until Dawn" (1 vé)
  const tomorrow = allShowDates[1];
  const show2 = allShows[tomorrow][2];
  const booking2 = {
    _id: "booking_dynamic_002",
    user: { name: " Công Truyền" },
    show: {
      _id: show2.showId,
      movie: dummyShowsData[1],
      showDateTime: show2.time,
      showPrice: pricePerSeat,
    },
  ...
  // --- Booking 3: Ngày mốt, suất 21:00, Phim "Lilo & Stitch" (3 vé)
  const dayAfter = allShowDates[2];
  const show3 = allShows[dayAfter][4];
  const booking3 = {
    _id: "booking_dynamic_003",
    user: { name: "Dũng Đẹp Trai" },
    show: {
      _id: show3.showId,
      movie: dummyShowsData[2],
      showDateTime: show3.time,
      showPrice: pricePerSeat,
    },
    amount: pricePerSeat * 3,
    bookedSeats: ["G1", "G2", "G3"],
    isPaid: false,
  };

  // 3. Trả về mảng các booking động (SỬA CÚ PHÁP)
  return [booking1, booking2, booking3]; // <-- Sửa dòng này
};
/*  file: client/src/assets/assets.js
nói về 
danh sách trailer (image+video url)
danh sách diễn viên (dummyCastsData)
danh sách phim đang chiếu (dummyShowData)\
danh sách thời gian chiếu (dummyDateTimeData)
dữ liệu dashboard (dummyDashboardData)( dùng cho admin)
    dữ liệu đặt vé (dummyBookingData) (dùng cho user đặt )
